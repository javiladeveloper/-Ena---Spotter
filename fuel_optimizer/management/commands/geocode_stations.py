"""
Management command to geocode fuel stations using OpenRouteService API.
"""
import time
import requests
...
# State centroids as fallback (approximate centers)
STATE_CENTROIDS = {
...
class Command(BaseCommand):
    help = "Geocode fuel stations using OpenRouteService API"

    def add_arguments(self, parser):
        parser.add_argument(
            "--use-state-centroids",
            action="store_true",
            help="Use state centroids instead of API geocoding (fast but less accurate)",
        )
        parser.add_argument(
            "--limit",
            type=int,
            default=0,
            help="Limit number of stations to geocode (0 for all)",
        )
    ...
    def geocode_with_ors(self, city: str, state: str) -> tuple:
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(f"Geocoding error for {city}, {state}: {e}")
            )
    ...
    def handle(self, *args, **options):
        ...
        if use_centroids:
            # Fast mode: use state centroids with small random offset
            ...
            for state_info in state_counts:
                ...
                if state in STATE_CENTROIDS:
                    ...
                    # Update all stations in this state
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"No centroid for state: {state}")
                    )

        else:
            # Accurate mode: geocode by city/state
            city_cache = {}
            geocoded = 0
            failed = 0
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Geocoding complete: {geocoded} successful, {failed} failed"
                )
            )
